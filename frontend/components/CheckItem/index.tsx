...
const CheckItem = ({ imgURL, label, checked = false, onChange, disabled, trailingTag }: CheckItemProps) => {
  ...
  return (
    <div className={`group/check-item Component relative rounded-md bg-ground box-content px-1 Transition_500 transition-colors border-2 border-solid ${isChecked ? 'border-primary' : 'border-transparent'} ${opacityClassName} ${hoverClassName}`}>
        {Content}
        <input type="checkbox" className="absolute inset-0 z-overlay opacity-0 cursor-pointer" disabled={disabled} checked={isChecked} onChange={handleChange} />
    </div>
  );
};
